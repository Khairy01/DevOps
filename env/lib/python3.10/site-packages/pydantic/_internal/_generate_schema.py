...
class GenerateSchema:
        ...
        with self.defs.get_schema_or_ref(cls) as (model_ref, maybe_schema):
            ...
            with self._config_wrapper_stack.push(config_wrapper):
                ...
                if cls.__pydantic_root_model__:
                    ...
                    inner_schema = define_expected_missing_refs(
    ...
    def _generate_schema_from_property(
        ...
        if get_schema is None:
            ...
            schema = core_schema.chain_schema(
                [core_schema.general_plain_validator_function(v) for v in validators()]
            )
        else:
    ...
    def _common_field_schema(
        ...
        metadata = build_metadata_dict(
            js_annotation_functions=[json_schema_update_func]
        )

        # apply alias generator
        alias_generator = self._config_wrapper.alias_generator
        if alias_generator and (
            field_info.alias_priority is None or field_info.alias_priority <= 1
        ):
    ...
    def _typed_dict_schema(
        ...
        with self.defs.get_schema_or_ref(typed_dict_cls) as (
            ...
            with self._config_wrapper_stack.push(config):
                ...
                metadata = build_metadata_dict(
                    js_functions=[partial(modify_model_json_schema, cls=typed_dict_cls)]
                )
    ...
    def _sequence_schema(self, sequence_type: Any) -> core_schema.CoreSchema:
        ...
        def json_schema_func(
            _schema: CoreSchemaOrField, handler: GetJsonSchemaHandler
        ) -> JsonSchemaValue:
    ...
    def _prepare_annotations(
        self, source_type: Any, annotations: Iterable[Any]
    ) -> tuple[Any, list[Any]]:
        ...
        annotations = tuple(
            annotations
        )  # make them immutable to avoid confusion over mutating them

        if prepare is not None:
            source_type, annotations = prepare(
                source_type, tuple(annotations), self._config_wrapper.config_dict
            )
    ...
    def _apply_annotations(
        ...
        while True:
            ...
            if prepare is not None:
                ...
                new_source_type, remaining = prepare(
                    source_type, tuple(remaining), self._config_wrapper.config_dict
                )
        ...
        if isinstance(metadata, FieldInfo):
            ...
            if json_schema_update or json_schema_extra:
                ...
                CoreMetadataHandler(schema).metadata.setdefault(
                    "pydantic_js_annotation_functions", []
                ).append(json_schema_update_func)
        return schema
...
_VALIDATOR_F_MATCH: Mapping[
    ...
    Callable[
        [Callable[..., Any], None, core_schema.CoreSchema], core_schema.CoreSchema
    ],
] = {
    (
        "before",
        "no-info",
    ): lambda f, _, schema: core_schema.no_info_before_validator_function(f, schema),
    (
        "after",
        "no-info",
    ): lambda f, _, schema: core_schema.no_info_after_validator_function(f, schema),
    (
        "plain",
        "no-info",
    ...
    (
        "wrap",
        "no-info",
    ): lambda f, _, schema: core_schema.no_info_wrap_validator_function(f, schema),
    (
        "before",
        "general",
    ): lambda f, _, schema: core_schema.general_before_validator_function(f, schema),
    (
        "after",
        "general",
    ): lambda f, _, schema: core_schema.general_after_validator_function(f, schema),
    (
        "plain",
        "general",
    ): lambda f, _1, _2: core_schema.general_plain_validator_function(f),
    (
        "wrap",
        "general",
    ): lambda f, _, schema: core_schema.general_wrap_validator_function(f, schema),
}
...
_FIELD_VALIDATOR_F_MATCH: Mapping[
    ...
    (
        "plain",
        "field",
    ): lambda f, field_name, _: core_schema.field_plain_validator_function(
        f, field_name
    ),
...
def apply_validators(
    ...
    for validator in validators:
        ...
        if not info_arg:
            ...
            validator.info, (FieldValidatorDecoratorInfo, ValidatorDecoratorInfo)
        ):
            assert (
                field_name is not None
            ), "field validators must be used within a model field"
            val_type = "field"
        else:
            val_type = "general"
        ...
        if field_name is None or val_type != "field":
            schema = _VALIDATOR_F_MATCH[(validator.info.mode, val_type)](
                validator.func, None, schema
            )
        else:
            schema = _FIELD_VALIDATOR_F_MATCH[(validator.info.mode, val_type)](
                validator.func, field_name, schema
            )
    return schema
