...
class _SecretField(Generic[SecretType]):
    ...
    def __prepare_pydantic_annotations__(
        ...
        _known_annotated_metadata.check_metadata(
            metadata, {"min_length", "max_length"}, cls
        )
        return (
            source,
            (
...
class _SecretFieldValidator:
    ...
    def validate(
        self,
        value: _SecretField[SecretType] | SecretType,
        _: core_schema.ValidationInfo,
    ) -> Any:
        error_prefix: Literal["string", "bytes"] = (
            "string" if issubclass(self.field_type, SecretStr) else "bytes"
        )
    ...
    def __get_pydantic_core_schema__(
        ...
        self.inner_schema = handler(
            str if issubclass(self.field_type, SecretStr) else bytes
        )
        error_kind = (
            "string_type" if issubclass(self.field_type, SecretStr) else "bytes_type"
        )
        return core_schema.general_after_validator_function(
            self.validate,
...
class GetPydanticSchema:
    ...
    get_pydantic_core_schema: Callable[
        [Any, _annotated_handlers.GetCoreSchemaHandler], CoreSchema
    ] | None = None
    get_pydantic_json_schema: Callable[
        [Any, _annotated_handlers.GetJsonSchemaHandler], JsonSchemaValue
    ] | None = None
