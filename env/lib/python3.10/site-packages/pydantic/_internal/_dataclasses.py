...
def complete_dataclass(
    ...
    *,
    ...
    # debug(schema)
    cls.__pydantic_core_schema__ = schema = _discriminated_union.apply_discriminators(
        flatten_schema_defs(schema)
    )
    simplified_core_schema = inline_schema_defs(schema)
    cls.__pydantic_validator__ = validator = SchemaValidator(
        simplified_core_schema, core_config
    )
...
def generate_dataclass_signature(cls: type[StandardDataclass]) -> Signature:
    ...
    for param in sig.parameters.values():
        ...
        if isinstance(param_default, FieldInfo):
            ...
            name = param.name
            if param_default.validation_alias is None and isinstance(
                param_default.alias, str
            ):
