...
class ValidateCallWrapper:
    ...
    def __init__(
        ...
        self.__pydantic_core_schema__ = schema = gen_schema.collect_definitions(
            gen_schema.generate_schema(function)
        )
        ...
        self.__pydantic_validator__ = pydantic_core.SchemaValidator(
            simplified_schema, core_config
        )
        ...
        if self._validate_return:
            ...
            gen_schema = _generate_schema.GenerateSchema(config_wrapper, namespace)
            self.__return_pydantic_core_schema__ = (
                schema
            ) = gen_schema.collect_definitions(gen_schema.generate_schema(return_type))
            core_config = config_wrapper.core_config(self)
            schema = _discriminated_union.apply_discriminators(
                flatten_schema_defs(schema)
            )
            simplified_schema = inline_schema_defs(schema)
